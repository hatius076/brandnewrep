/**
 * Enhanced Visual Novel Game Engine with Warden AI Architecture
 * Complete overhaul implementing dynamic fact collection and user-controlled quiz generation
 */
...
class VisualNovelGame {
    constructor() {
        this.state = {
            ...
            startTime: Date.now(),
            playerFacts: {}, // Legacy - now handled by Warden
            quizAnswers: [],
            ratings: {},
            dialogue: [],
            memoryErrors: 0,
            ...
            debugMode: false,
            lastLLMThought: '',
            // New Warden-related state
            wardenEnabled: GAME_CONFIG.WARDEN?.ENABLED || true,
            userQuestionsAsked: 0,
            quizPhaseActive: false
        };
        
        this.elements = {};
        this.warden = null; // Warden AI instance
        this.initializeElements();
        this.initializeEventListeners();
        this.initializeLLMSystem();
        this.initializeWarden();
        this.startGame();
    }
    ...
    initializeElements() {
        // Cache DOM elements - including new quiz elements
        this.elements = {
            ...
            // Updated quiz elements
            quizContainer: document.getElementById('quiz-container'),
            quizIntroText: document.getElementById('quiz-intro-text'),
            userQuestionContainer: document.getElementById('user-question-container'),
            userQuestionInput: document.getElementById('user-question-input'),
            askQuestionButton: document.getElementById('ask-question-button'),
            quizResponseContainer: document.getElementById('quiz-response-container'),
            quizResponseText: document.getElementById('quiz-response-text'),
            askAnotherQuestion: document.getElementById('ask-another-question'),
            finishQuiz: document.getElementById('finish-quiz'),
            
            // Legacy quiz elements (hidden but kept for compatibility)
    ...
    initializeEventListeners() {
        ...
        // New quiz event listeners
        this.elements.askQuestionButton.addEventListener('click', () => this.handleUserQuestion());
        this.elements.userQuestionInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') this.handleUserQuestion();
        });
        this.elements.askAnotherQuestion.addEventListener('click', () => this.showUserQuestionInput());
        this.elements.finishQuiz.addEventListener('click', () => this.finishQuizPhase());
